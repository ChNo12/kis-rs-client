...
impl<T: HttpClient> Service<'_, T> {
    ...
    pub async fn after_hour_balance_pages(
        &self,
        access_token: &AccessToken,
        mut request: AfterHourBalanceRequest,
        limit: PageLimit,
    ) -> Result<PageCollection<AfterHourBalanceResponse, Continuation>> {
        let max_pages = limit.max_pages()?;
        let mut pages = Vec::new();

        loop {
            let response = match self.after_hour_balance(access_token, request.clone()).await {
